...
model_top <- multinom(quality_binary ~ alcohol + fixed.acidity + volatile.acidity + sulphates, data = train)
...
pred_nodensity_sugar <- factor(predict(model_nodensity_sugar, newdata = test), levels = levels(test$quality_binary))
